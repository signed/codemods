...
describe('extractImportFrom', () => {
  test('identify re export all exports', () => {
    expect(importIn(`export * from './sample'`)).toStrictEqual<Import>({
      importString: './sample',
      imported: 'all-exports',
    })
  })
  test('identify re export specific exports', () => {
    expect(importIn(`export {one, two} from './sample'`)).toStrictEqual<Import>({
      importString: './sample',
      imported: ['one', 'two'],
    })
  })
  ...
  test('identify default imports', () => {
    expect(importIn(`import anything from './sample'`)).toStrictEqual<Import>({
      importString: './sample',
      imported: ['default'],
    })
  })
  test('identify named import', () => {
    expect(importIn(`import {one, two} from './sample'`)).toStrictEqual<Import>({
      importString: './sample',
      imported: ['one', 'two'],
    })
  })
  test('identify mixed named and default import', () => {
    expect(importIn(`import one, {two} from './sample'`)).toStrictEqual<Import>({
      importString: './sample',
      imported: ['two', 'default'],
    })
  })
  test('identify renamed named import', () => {
    expect(importIn(`import {one as two} from './sample'`)).toStrictEqual<Import>({
      importString: './sample',
      imported: ['one'],
    })
  })
...
describe('extractExportsFrom', () => {
  test('identify default export', () => {
    expect(exportIn(`export default 42`)).toStrictEqual<Export>({
      exportString: 'default',
    })
  })
  ...
  test.each([
    ...
    expect(exportsIn(source)).toContainEqual<Export>({
      exportString: 'default',
    })
  })
  ...
  test('identify named exports', () => {
    expect(
      exportsIn(`export const one = 42
export const two = 2`),
    ).toStrictEqual<Export[]>([{ exportString: 'one' }, { exportString: 'two' }])
  })

  test('only extract the identifier of the export ', () => {
    expect(exportIn(`export const one = (notAnExport:string) => {}`)).toStrictEqual<Export>({ exportString: 'one' })
  })
  test('extract interface exports', () => {
    expect(exportIn(`export interface One {}`)).toStrictEqual<Export>({
      exportString: 'One',
    })
  })
  test('extract class exports', () => {
    expect(exportIn(`export class One {}`)).toStrictEqual<Export>({
      exportString: 'One',
    })
  })
  test('extract enum exports', () => {
    expect(exportIn(`export enum One { one, two}`)).toStrictEqual<Export>({
      exportString: 'One',
    })
  })
  test('extract type exports', () => {
    expect(exportIn(`export type One = string`)).toStrictEqual<Export>({
      exportString: 'One',
    })
  })
  test('extract re export', () => {
    expect(exportIn(`export { One } from '../sample'`)).toStrictEqual<Export>({
      exportString: 'One',
    })
  })
  test('extract renamed re export', () => {
    expect(exportIn(`export { One as Two } from '../sample'`)).toStrictEqual<Export>({ exportString: 'Two' })
  })
  test('extract function export', () => {
    expect(
      exportIn(`export async function one() {
  return undefined;
}`),
