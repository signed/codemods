...
const walk = (directory: string, descent: (path: string) => boolean, acc: string[] = []): string[] => {
...
export const extractImportsFrom = (source: string, j: JSCodeshift): Import[] => {
  ...
  root.find(j.ExportNamedDeclaration).forEach((exportNamedDeclaration) => {
    ...
    const imported = j(exportNamedDeclaration)
...
export const probeForDeadCodeIn = (projectDirectory: string): Unused => {
  ...
  const allFilesInProject = walk(projectDirectory, descent).filter(
    ...
    const imports: ResolvedImport[] = extractImportsFrom(source, j)
      ...
      .map((it) => {
        ...
        const candidates = [
          ...
          absolutePath + '.js',
          absolutePath + '.jsx',
          absoluteIndexPath + '.ts',
          absoluteIndexPath + '.tsx',
          absoluteIndexPath + '.js',
          absoluteIndexPath + '.jsx',
        ]
  ...
  Array.from(usageLedger.entries()).forEach(([sourceFile, entry]) => {
